...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h3 mb-0">Edit URL Redirect</h1>
        ...
        <form action="{{ route('admin.redirects.update', $redirect) }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-5 mb-3">
                    ...
                    <input type="text" name="old_url" class="form-control @error('old_url') is-invalid @enderror" value="{{ old('old_url', $redirect->old_url) }}" required>
                    @error('old_url')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-5 mb-3">
                    <label class="form-label fw-semibold">New URL *</label>
                    <input type="text" name="new_url" class="form-control @error('new_url') is-invalid @enderror" value="{{ old('new_url', $redirect->new_url) }}" required>
            ...
            <div class="form-check form-switch mb-3">
                ...
                <input class="form-check-input" type="checkbox" name="status" value="1" id="status" {{ old('status',$redirect->status)?'checked':'' }}>
                <label class="form-check-label" for="status">Active</label>
            </div>
            <hr>
            <button type="submit" class="btn btn-primary"><i class="fas fa-save me-1"></i> Update Redirect</button>
