...
@section('title', $category->name . ' — Plantable Products')
...
.prod-img-box {
    position:relative;background:#f8fdf8;height:230px;
    display:flex;align-items:center;justify-content:center;
    overflow:hidden;flex-shrink:0;
}
.prod-img-box img {
    max-height:210px;max-width:90%;width:auto;height:auto;
...
.prod-price-val { font-size:1.25rem;font-weight:800;color:#2E7D32; }
.prod-btn {
    display:block;width:100%;padding:11px 0;
    background:#2E7D32;color:#fff;border:none;border-radius:10px;
    font-weight:700;text-align:center;text-decoration:none;font-size:14px;transition:background .2s;
}
    ...
    <div class="container">
        <nav aria-label="breadcrumb" style="margin-bottom:10px;">
            <ol class="breadcrumb mb-0" style="background:transparent;padding:0;">
                ...
                <li class="breadcrumb-item"><a href="{{ route('frontend.categories.index') }}" style="color:rgba(255,255,255,.7);text-decoration:none;">Categories</a></li>
...
<section style="padding:60px 0;background:#f8fdf8;">
    <div class="container">
        ...
        <div class="mb-5 p-4 bg-white rounded-3 border" style="border-color:rgba(46,125,50,.15)!important;color:#555;line-height:1.8;">
        ...
        <div class="row g-4">
            ...
            <div class="col-sm-6 col-lg-4 col-xl-3">
                <div class="prod-card">
                    ...
                    <div class="prod-body">
                        <p class="prod-cat"><i class="fas fa-seedling me-1"></i>{{ $product->seed_type ?? $category->name }}</p>
                        <h3 class="prod-name">{{ $product->name }}</h3>
                        <p class="prod-desc">{{ $product->short_description }}</p>
                        <div class="prod-price-row">
                            @if($product->sale_price)
                                <span class="prod-old">₹{{ number_format($product->price,0) }}</span>
                                <span class="prod-price-val">₹{{ number_format($product->sale_price,0) }}</span>
                            @else
                                <span class="prod-price-val">₹{{ number_format($product->final_price,0) }}</span>
                            @endif
                        </div>
